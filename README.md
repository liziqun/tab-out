...
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/liziqun/tab-out/main/install.sh)"
```

脚本会自动完成以下操作：
1. 克隆仓库到 `~/tab-out`（已存在则自动更新）
2. 打开 Chrome 扩展管理页面 `chrome://extensions`
3. 提示你手动开启 **开发者模式** 并加载 `extension/` 文件夹

> **注意**：由于 Chrome 安全限制，扩展加载步骤需要手动完成。脚本会为你打开对应页面并给出提示。
